...
class Code_To_Tokens_Converter {
    ...
    static convert_code_to_tokens(code: string) {
        ...
        while (this.there_are_characters_left_to_process) {
            ...
            if(possible_token_that_starts_at_current_char_index === false) {
                throw new Error("I think token here must always be non-false, right?")
            }
    ...
    static orAnIntegerConstant(): false | Token {
        let token: false | Token = false;
    ...
    static orAnIdentifier(): Token | false  {
        let token: Token | false  = false;
    ...
    static character_is_number_or_underscore_or_ascii_english_letter(character: string) {
    ...
    static does_character_match_any_keyword_character_at_index(character: string, keyword_character_index: number) {
